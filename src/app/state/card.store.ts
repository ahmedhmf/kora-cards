...
interface CardState{experience:Experience;stage:CardStage;filterStage:FilterStage;filterImageUrl:string;filterProcessed:boolean;cardMediaType:'image'|'video';countdown:number;error:string;isSharing:boolean;driveAvailable:boolean;driveUploading:boolean;driveMessage:string;cardLoading:boolean;playerName:string;playerPosition:string;playerNumber:number;cardDesign:CardDesign;replayUrl:string;mediaStream:MediaStream|null;matchedPlayer:RatedPlayer}
const initialState:CardState={experience:'home',stage:'intro',filterStage:'source',filterImageUrl:'',filterProcessed:false,cardMediaType:'video',countdown:10,error:'',isSharing:false,driveAvailable:!GOOGLE_DRIVE_CONFIG.clientId.startsWith('PASTE_'),driveUploading:false,driveMessage:'',cardLoading:false,playerName:'',playerPosition:'ST',playerNumber:10,cardDesign:'blue-gold',replayUrl:'',mediaStream:null,matchedPlayer:randomPlayer('ST')};
export const CardStore=signalStore(
 ...
 withMethods((store,camera=inject(CameraService),renderer=inject(CardRendererService),exporter=inject(CardExportService),filter=inject(PhotoFilterService),drive=inject(GoogleDriveService))=>{
  ...
  return{
   ...
   async openFilterCamera(){patchState(store,{error:'',cardLoading:true});try{const mediaStream=await camera.open();patchState(store,{filterStage:'camera',mediaStream})}catch{patchState(store,{error:'Camera access is needed. Allow it in your browser settings.',cardLoading:false})}},
   async startLiveFilter(video:HTMLVideoElement,canvas:HTMLCanvasElement){try{await filter.startLive(video,canvas)}catch(error){console.error(error);patchState(store,{error:'The live KORA effect could not start. Check your connection and try again.'})}finally{patchState(store,{cardLoading:false})}},
   ...
   async retake(){renderer.stop();if(clipUrl){URL.revokeObjectURL(clipUrl);clipUrl=''}patchState(store,{replayUrl:''});try{const mediaStream=await camera.open();patchState(store,{stage:'camera',mediaStream})}catch{patchState(store,{error:'Camera access is needed.',stage:'intro'})}},
