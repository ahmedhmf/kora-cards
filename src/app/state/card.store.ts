...
export const CardStore=signalStore(
 ...
 withMethods((store,camera=inject(CameraService),renderer=inject(CardRendererService),exporter=inject(CardExportService),filter=inject(PhotoFilterService),drive=inject(GoogleDriveService))=>{
  ...
  return{
   ...
   async openCamera(){patchState(store,{error:''});try{const mediaStream=await camera.open(store.facingMode());patchState(store,{stage:'camera',mediaStream})}catch{patchState(store,{error:'Camera access is needed. Allow it in your browser settings.'})}},
