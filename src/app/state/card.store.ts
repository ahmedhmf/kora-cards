...
interface CardState{experience:Experience;stage:CardStage;filterStage:FilterStage;facingMode:FacingMode;filterImageUrl:string;filterProcessed:boolean;cardMediaType:'image'|'video';countdown:number;error:string;isSharing:boolean;driveAvailable:boolean;driveUploading:boolean;driveMessage:string;cardLoading:boolean;playerName:string;instagramHandle:string;playerPosition:string;playerNumber:number;cardDesign:CardDesign;replayUrl:string;mediaStream:MediaStream|null;matchedPlayer:RatedPlayer}
const initialState:CardState={experience:'card',stage:'intro',filterStage:'source',facingMode:'environment',filterImageUrl:'',filterProcessed:false,cardMediaType:'video',countdown:3,error:'',isSharing:false,driveAvailable:!GOOGLE_DRIVE_CONFIG.clientId.startsWith('PASTE_'),driveUploading:false,driveMessage:'',cardLoading:false,playerName:'',instagramHandle:'',playerPosition:'ST',playerNumber:10,cardDesign:'blue-gold',replayUrl:'',mediaStream:null,matchedPlayer:randomPlayer('ST')};
export const CardStore=signalStore(
 ...
 withMethods((store,camera=inject(CameraService),renderer=inject(CardRendererService),exporter=inject(CardExportService),filter=inject(PhotoFilterService),drive=inject(GoogleDriveService))=>{
  ...
  return{
   ...
   setInstagramHandle(instagramHandle:string){patchState(store,{instagramHandle})},
   ...
   async renderCard(video:HTMLVideoElement,canvas:HTMLCanvasElement){patchState(store,{cardLoading:true,error:''});try{await renderer.start(video,canvas,()=>({name:store.playerName(),instagramHandle:store.instagramHandle(),position:store.playerPosition(),number:store.playerNumber(),design:store.cardDesign(),match:store.matchedPlayer(),facingMode:store.facingMode()}))}catch(error){console.error(error);patchState(store,{error:'Card renderer could not start. Please reload.'})}finally{patchState(store,{cardLoading:false})}},
   async renderCardPhoto(image:HTMLImageElement,canvas:HTMLCanvasElement){patchState(store,{cardLoading:true,error:''});try{await renderer.renderImage(image,canvas,()=>({name:store.playerName(),instagramHandle:store.instagramHandle(),position:store.playerPosition(),number:store.playerNumber(),design:store.cardDesign(),match:store.matchedPlayer(),facingMode:store.facingMode()}))}catch(error){console.error(error);patchState(store,{error:'Card renderer could not start. Please reload.'})}finally{patchState(store,{cardLoading:false})}},
   ...
   async startLivePreview(video:HTMLVideoElement,canvas:HTMLCanvasElement){try{await renderer.startLivePreview(video,canvas,()=>({name:store.playerName(),instagramHandle:store.instagramHandle(),position:store.playerPosition(),number:store.playerNumber(),design:store.cardDesign(),match:store.matchedPlayer(),facingMode:store.facingMode()}))}catch(e){console.error(e)}},
   async shareCardVideo(canvas:HTMLCanvasElement){patchState(store,{isSharing:true,error:''});try{const rawHandle=store.instagramHandle().trim(),handle=rawHandle?(rawHandle.startsWith('@')?rawHandle:`@${rawHandle}`):'',target=handle||store.playerName().trim()||'KORA STAR',shareText=`Created with KORA for ${target} ⚽`;await exporter.shareVideo(canvas,`${store.playerName()||'star'}-card`,'My KORA Player Card',shareText)}catch(error){if((error as DOMException).name!=='AbortError')patchState(store,{error:'Video export is not supported by this browser. Try downloading the image instead.'})}finally{patchState(store,{isSharing:false})}},
